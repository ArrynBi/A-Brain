...
如果你想马上体验 ingest / think / learn 的联动，可以加上：
...
- 立即派生 `turn` / `session` 视图
- 按参数创建第一份 `knowledge/projects` 页面
- 可选复制 examples 并导入 sample source
- 运行 `think-refresh` 与 `think-health`
- 在使用 examples 时顺带跑一轮 `dream-maintain`

如果你更习惯手动控制，也可以按下面的分步流程执行。
...
自检会复制一份临时项目，验证 diary、think、ingest、dream、learn 的关键命令，并检查不会创建 `learn/reviews` 或 `.codex/skills`。当前仓库如果已经写入过 diary event、report 或其他运行产物，也可以直接运行。
